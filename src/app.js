...
const excessSpace = (value) => REG_NAME_SPACE.test(value) ? "No extra spaces allowed! Please try again." : "";

const specialCharacters = (value) => REG_NAME_SPECIAL_CHARACTERS.test(value) ? "" : "Special characters are not allowed!";

const min = (numBound) => (value) => value.length >= numBound ? "" : `Minimum ${numBound} characters!`;

const max = (numBound) => (value) => value.length <= numBound ? "" : `Maximum ${numBound} characters!`;

const isNagative = (value) => REG_NEGATIVE.test(value) ? "Negative numbers are not allowed!" : "";

const isNumber = (value) => REG_ISNUMBER.test(value) ? "" : "Don't use letters and make sure is a number!";

const isInteger = (value) => REG_ISINTEGER.test(value) ? "" : "Only integers are accepted!";
...
const clearMsg = () => {
    let errMsg = [...document.querySelectorAll(".error-input"), ...document.querySelectorAll(".error-input-none-img"), ...document.querySelectorAll(".error-msg")];
...
RenderUI.prototype.add = function ({ id, name, price, quantity, media, description }) {
    ...
    productItem.innerHTML = `
        ...
        <div class="product-info" data-id="${id}">
            ...
            <div class="product-price-item" data-id="${id}">
                ...
                <p class="product-price-discount" data-id="${id}">${price * 2}</p>
            </div>
        </div>
    `;
...
document.querySelector("#delete-product").addEventListener("click", (event) => {
    if (selectedProduct !== null) {
        const { id, name, price, quantity, media, description } = selectedProduct[0];
...
//Remove All
document.querySelector("#remove-all-product").addEventListener("click", (event) => {
